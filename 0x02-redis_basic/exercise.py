...
class Cache:
    ...
    def store(self: 'Cache', data: Any) -> str:
        """Stores data in redis using a key."""
        key = str(uuid4())
        self._redis.set(key, data)
        return key

    def get(self: 'Cache', key: str, fn: Callable | None = None) -> Any | None:
    ...
    def get_str(self: 'Cache', key: str) -> str | None:
        "Retrieves a string from reddis"
        return self.get(key, str)

    def get_int(self: 'Cache', key: str) -> int | None:
